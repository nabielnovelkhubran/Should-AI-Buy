...
export const DeliberationFeed: React.FC<DeliberationFeedProps> = ({
  ...
  const getVerdictBadge = (verdict: string, failed?: boolean) => {
    if (failed) {
      return <span className="px-2 py-0.5 rounded text-[11px] font-bold bg-rose-500/20 text-rose-400 border border-rose-500/30">FAILED</span>;
    }
    if (verdict === 'BUY' || verdict === 'VALID' || verdict === 'OPPORTUNITY') {
      return <span className="px-2 py-0.5 rounded text-[11px] font-bold bg-emerald-500/20 text-emerald-400 border border-emerald-500/30">{verdict}</span>;
    }
    if (verdict === 'REJECT' || verdict === 'DISPROVED') {
      return <span className="px-2 py-0.5 rounded text-[11px] font-bold bg-rose-500/20 text-rose-400 border border-rose-500/30">{verdict}</span>;
    }
    ...
    return <span className="px-2 py-0.5 rounded text-[11px] font-bold bg-slate-800 text-slate-300 border border-slate-700">{verdict}</span>;
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-4 rounded-2xl bg-[#11141d] border border-slate-800 space-y-3">
          ...
          {STAGE_CONFIG.map(({ stage, label, question, icon: IconComponent }) => {
            ...
            return (
              <button
                ...
                className={`p-2.5 rounded-xl border text-left flex flex-col justify-between transition cursor-pointer group ${
                  isSelected
              ...
              >
                <div>
                  <div className="flex items-center justify-between gap-1 mb-1.5">
                    <div className="flex items-center gap-1.5 min-w-0">
                      <IconComponent className={`w-3.5 h-3.5 shrink-0 ${
                        isSelected ? 'text-indigo-300' : isCompleted ? 'text-emerald-400' : isFailed ? 'text-rose-400' : isRunning ? 'text-indigo-400' : 'text-slate-500'
                      }`} />
                  ...
                  <div className={`text-[11px] text-slate-300 leading-snug transition-all ${
        ...
        {activeStageConfig && (
          <div className="p-4 rounded-xl bg-slate-900/90 border border-indigo-500/30 space-y-3 animate-fadeIn">
            ...
            <div className="text-xs text-slate-200 leading-relaxed">
              <strong className="text-slate-400 uppercase font-mono text-[10px] block mb-1">Stage Deliberation Output:</strong>
              <div className="p-3 rounded-lg bg-slate-950/60 border border-slate-800 text-slate-200">
      ...
      {decision && (
        <div className={`p-6 rounded-2xl border ${
          isRiskGateBlocked
            ? 'bg-gradient-to-r from-amber-950/40 via-[#11141d] to-[#11141d] border-amber-500/40 glow-redteam'
            : decision.conclusion === 'BUY'
            ? 'bg-gradient-to-r from-emerald-950/40 via-[#11141d] to-[#11141d] border-emerald-500/40 glow-bullish'
            : decision.conclusion === 'REJECT'
            ? 'bg-gradient-to-r from-rose-950/40 via-[#11141d] to-[#11141d] border-rose-500/40 glow-redteam'
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div>
              ...
              <div className="flex flex-wrap items-center gap-3 mt-1.5">
                <span className={`text-3xl font-extrabold tracking-tight ${
                  decision.conclusion === 'BUY' ? 'text-emerald-400' :
                  decision.conclusion === 'REJECT' ? 'text-rose-400' :
                  'text-amber-400'
                }`}>
            ...
            <div className="flex items-center gap-4 bg-slate-900/80 p-3.5 rounded-xl border border-slate-800">
              <div className="text-center">
                <div className="text-[10px] text-slate-400 uppercase font-semibold">Opportunity</div>
                <div className="text-lg font-bold text-indigo-400">{decision.opportunityScore}/100</div>
              </div>
              <div className="h-8 w-px bg-slate-800" />
              <div className="text-center">
                <div className="text-[10px] text-slate-400 uppercase font-semibold">Risk Score</div>
                <div className={`text-lg font-bold ${decision.riskScore > 70 ? 'text-rose-400' : 'text-slate-300'}`}>
                  {decision.riskScore}/100
                </div>
              </div>
              <div className="h-8 w-px bg-slate-800" />
              <div className="text-center">
                <div className="text-[10px] text-slate-400 uppercase font-semibold">Risk Gate</div>
                <div className={`text-xs font-bold mt-1 px-2.5 py-0.5 rounded ${
                  decision.riskGateApproved ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'
                }`}>
          ...
          {decision.strongestCounterargument && (
            <div className="mt-2.5 text-xs text-amber-300 flex items-start gap-2 bg-amber-950/20 p-2.5 rounded-xl border border-amber-500/20">
          ...
          {isRiskGateBlocked && (
            <div className="mt-3.5 p-3.5 rounded-xl bg-rose-950/40 border border-rose-500/40 text-xs text-rose-300 space-y-1">
              ...
              {decision.riskGateNotes?.map((note, i) => (
                <div key={i} className="pl-5 text-rose-300 font-semibold">• {note}</div>
              ))}
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {agentRuns['discovery'] && (
          <div className="p-4 rounded-2xl bg-[#11141d] border border-slate-800 flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between mb-2.5">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <span className="text-[10px] text-slate-500">What is happening?</span>
                  </div>
                </div>
            ...
            <div className="mt-3 pt-3 border-t border-slate-800/60 flex items-center justify-between text-[11px] text-slate-400">
              <span>Opp Score: <strong className="text-indigo-400">{agentRuns['discovery'].metrics?.opportunityScore}/100</strong></span>
              {onViewEvidence && (
                <button
              ...
              <div className="flex items-center justify-between mb-2.5">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <span className="text-[10px] text-slate-500">What do numbers say?</span>
                  </div>
                </div>
                {getVerdictBadge(agentRuns['quant'].verdict, agentRuns['quant'].failed)}
              </div>

              <div className="grid grid-cols-3 gap-1.5 my-2.5 p-2 rounded-xl bg-slate-900/60 border border-slate-800 text-center text-[11px]">
                <div>
              ...
              <div className="flex items-center justify-between mb-2.5">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <span className="text-[10px] text-slate-500">What external news exists?</span>
                  </div>
                </div>
              ...
              <div className="flex items-center justify-between mb-2.5">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <span className="text-[10px] text-slate-500">What could go wrong?</span>
                  </div>
                </div>
                {getVerdictBadge(agentRuns['risk'].verdict, agentRuns['risk'].failed)}
              </div>

              <div className="grid grid-cols-2 gap-2 my-2.5 p-2 rounded-xl bg-slate-900/60 border border-slate-800 text-center text-[11px]">
                <div>
                  <span className="text-[10px] text-slate-500 block">Risk Score</span>
                  <strong className={`font-mono ${Number(agentRuns['risk'].metrics?.compositeRiskScore) > 60 ? 'text-rose-400' : 'text-white'}`}>
                    {agentRuns['risk'].metrics?.compositeRiskScore ?? 40}/100
                  </strong>
                </div>
                <div>
                  <span className="text-[10px] text-slate-500 block">Top 10 Supply</span>
        ...
        {agentRuns['red_team'] && (
          <div className="p-4 rounded-2xl bg-[#11141d] border border-slate-800 flex flex-col justify-between">
              ...
              <div className="flex items-center justify-between mb-2.5">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <span className="text-[10px] text-slate-500">Why might we be wrong?</span>
                  </div>
                </div>
            ...
            <div className="mt-3 pt-3 border-t border-slate-800/60 flex items-center justify-between text-[11px] text-slate-400">
              <span>Thesis: <strong className="font-mono text-rose-300">{agentRuns['red_team'].redTeamAttackDetails?.thesisStatus}</strong></span>
              {onViewEvidence && (
                <button
                  onClick={() => onViewEvidence('RISK')}
        ...
        <div className="space-y-2 max-h-56 overflow-y-auto pr-1 divide-y divide-slate-800/40">
          {timeline.map((event, idx) => (
            <div key={idx} className="pt-2 first:pt-0 flex items-start gap-3 text-xs">
              <span className="font-mono text-slate-500 shrink-0 text-[11px]">{event.timestamp}</span>
              <span className="font-mono text-indigo-400 uppercase font-semibold shrink-0">[{event.agent}]</span>
