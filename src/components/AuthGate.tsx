...
import { Lock, Unlock, Eye, EyeOff, ShieldCheck, AlertCircle, RefreshCw, KeyRound, Sparkles } from 'lucide-react';
...
export const AuthGate: React.FC<AuthGateProps> = ({ children }) => {
  ...
  // Fast initial check on client
  useEffect(() => {
    const checkAuth = async () => {
      if (typeof document !== 'undefined' && document.cookie.includes('saib_session=')) {
        try {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!password.trim()) {
      setError('Please enter the operator passphrase.');
      return;
    }
      ...
      <div className="min-h-screen bg-[#0b0a0e] flex flex-col items-center justify-center p-4 relative overflow-hidden">
        ...
        <div className="w-full max-w-md relative z-10">
          ...
          <div className="bg-[#121117] rounded-2xl border border-[#28272e] p-6 sm:p-8 shadow-2xl shadow-black/80 backdrop-blur-xl relative">
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div className="flex items-center justify-between text-xs font-mono">
                <label className="flex items-center gap-2 cursor-pointer text-[#848388] hover:text-slate-300 select-none">
                  ...
                  />
                  <span>Remember device (30-day cookie)</span>
                </label>
              </div>
            ...
            {/* Judge / Demo Hint */}
            <div className="mt-6 pt-4 border-t border-[#28272e] text-center font-mono text-[11px] text-[#848388]">
              <span>Judge &amp; Demo Passphrase: </span>
              <button
                type="button"
                onClick={() => setPassword('alpaca2026')}
                className="text-[#00ff84] font-bold underline hover:text-[#00e576] ml-1 select-all cursor-pointer"
              >
                alpaca2026
              </button>
            </div>
