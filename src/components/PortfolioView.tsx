  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-indigo-600/20 border border-indigo-500/40 text-xs font-semibold text-indigo-300 hover:bg-indigo-600/30 transition disabled:opacity-50"
          >
          ...
          <button
            ...
            disabled={isLoading}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-800 border border-slate-700 text-xs font-semibold text-slate-300 hover:bg-slate-700 hover:text-white transition disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="p-4 rounded-2xl bg-[#11141d] border border-slate-800 space-y-1">
          <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Portfolio Equity</span>
          <div className="text-xl font-bold font-mono text-white">
            {formatCurrency(acc?.equity || 100000)}
          </div>
          <div className="text-[10px] text-slate-500 flex items-center gap-1">
            <span>Buying Power:</span>
            <span className="font-mono text-slate-300">{formatCurrency(acc?.buyingPower || 200000)}</span>
          </div>
        </div>

        <div className="p-4 rounded-2xl bg-[#11141d] border border-slate-800 space-y-1">
          <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Cash Balance</span>
          <div className="text-xl font-bold font-mono text-white">
            {formatCurrency(acc?.cash || 100000)}
          </div>
          <div className="text-[10px] text-slate-500 flex items-center gap-1">
            <span>Currency:</span>
            <span className="font-mono text-slate-300">{acc?.currency || 'USD'}</span>
          </div>
        </div>

        <div className="p-4 rounded-2xl bg-[#11141d] border border-slate-800 space-y-1">
          <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Unrealized P&L</span>
          ...
          <div className={`text-[10px] font-mono ${totalPnlPct >= 0 ? 'text-emerald-400/80' : 'text-rose-400/80'}`}>
            {totalPnlPct >= 0 ? '+' : ''}{totalPnlPct.toFixed(2)}% total return
          </div>
        </div>

        <div className="p-4 rounded-2xl bg-[#11141d] border border-slate-800 space-y-1">
          <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Gross Exposure</span>
          <div className="text-xl font-bold font-mono text-white">
      ...
      <div className="p-6 rounded-2xl bg-[#11141d] border border-slate-800 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          {monitoringResult && (
            <div className="flex items-center gap-2 text-[11px] font-mono">
              <span className="text-emerald-400 font-semibold">{monitoringResult.healthyCount} Healthy</span>
              <span className="text-slate-600">•</span>
              <span className="text-amber-400 font-semibold">{monitoringResult.degradedCount} Degraded</span>
              <span className="text-slate-600">•</span>
              <span className="text-rose-400 font-semibold">{monitoringResult.invalidatedCount} Invalidated</span>
            </div>
          )}
        </div>
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                        rec.position.side === 'short' ? 'bg-rose-500/20 text-rose-300' : 'bg-emerald-500/20 text-emerald-300'
                      }`}>
                  ...
                  <div className="grid grid-cols-3 gap-2 mt-3 text-[11px] font-mono">
                    <div className="p-2 rounded bg-slate-900/80 border border-slate-800/80">
                      ...
                      <span className={`font-bold ${h.score >= 70 ? 'text-emerald-400' : h.score >= 40 ? 'text-amber-400' : 'text-rose-400'}`}>
                        {h.score}/100
                      </span>
                    </div>
                    <div className="p-2 rounded bg-slate-900/80 border border-slate-800/80">
                      <span className="text-slate-500 block text-[10px]">Position P&L</span>
                      <span className={`font-bold ${(rec.position.unrealizedPnl || 0) >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                  ...
                  {h.findings && h.findings.length > 0 && (
                    <div className="mt-3 space-y-1">
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Evaluation Findings</span>
                      <ul className="space-y-1 text-[11px] text-slate-300">
                        {h.findings.map((f, fIdx) => (
                          <li key={fIdx} className="flex items-start gap-1.5">
                            ...
                            <span className={f.severity === 'CRITICAL' ? 'text-rose-300' : 'text-amber-300'}>
                              {f.message}
                            </span>
                          </li>
                        ))}
                  ...
                  {rec.proposal && (
                    <div className="mt-3 pt-3 border-t border-slate-800/80 flex items-center justify-between">
                      <div>
                        <span className="text-[10px] text-slate-400 block font-semibold uppercase">Protective Action</span>
      ...
      <div className="p-6 rounded-2xl bg-[#11141d] border border-slate-800 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-slate-500">Authoritative Broker Fills</span>
        </div>
              ...
              <tbody className="divide-y divide-slate-800/60">
                {posList.map((pos, i) => (
                  <tr key={i} className="hover:bg-slate-900/40 transition">
                    ...
                    <td className="py-3 px-3 font-mono text-slate-300">${pos.avgEntryPrice?.toFixed(2) || '0.00'}</td>
      ...
      <div className="p-6 rounded-2xl bg-[#11141d] border border-slate-800 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-slate-500">Order Reconciliation Queue</span>
        </div>
              ...
              <tbody className="divide-y divide-slate-800/60">
                {ordList.map((ord, i) => (
                  <tr key={i} className="hover:bg-slate-900/40 transition">
                    ...
                    <td className="py-2.5 px-3 font-mono text-emerald-400">{ord.filledQty || 0}</td>
                    <td className="py-2.5 px-3 font-mono text-slate-400">{ord.remainingQty || ord.qty}</td>
                    <td className="py-2.5 px-3">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                        ord.status === 'FILLED'
                          ? 'bg-emerald-500/20 text-emerald-300'
