...
export const DiscoveryDashboard: React.FC<DiscoveryDashboardProps> = ({ onSelectInvestigation }) => {
  ...
  const handleRunScan = async () => {
    ...
    try {
      const res = await fetch('/api/discovery', {
        ...
        body: JSON.stringify({ autoDispatch: true, dispatchLimit: 1 })
      });
      const data = await res.json();
      if (res.ok) {
  ...
  const getQueueStatusBadge = (symbol: string) => {
    ...
    switch (item.status) {
      ...
      case 'INVESTIGATING':
        return <span className="px-2.5 py-1 rounded-lg text-[10px] font-bold tracking-wider bg-purple-500/10 text-purple-400 border border-purple-500/20 animate-pulse">INVESTIGATING</span>;
      case 'COMPLETED':
        return <span className="px-2.5 py-1 rounded-lg text-[10px] font-bold tracking-wider bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">COMPLETED</span>;
      case 'FAILED':
        return <span className="px-2.5 py-1 rounded-lg text-[10px] font-bold tracking-wider bg-rose-500/10 text-rose-400 border border-rose-500/20">FAILED</span>;
      case 'REJECTED':
        return <span className="px-2.5 py-1 rounded-lg text-[10px] font-bold tracking-wider bg-slate-500/10 text-slate-400 border border-slate-500/20">REJECTED</span>;
      default:
        return null;
    }
  };
  ...
  return (
    <div className="space-y-8">
      {/* 1. Header & Controls */}
      <div className="p-6 rounded-3xl bg-gradient-to-br from-[#11141d] via-[#111624] to-[#0c0f17] border border-slate-800 shadow-2xl relative overflow-hidden">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mt-6 pt-6 border-t border-slate-800/80">
          <div className="p-3.5 rounded-2xl bg-slate-900/60 border border-slate-800/60">
            <div className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Scan Universe</div>
            <div className="text-lg font-black text-white mt-1">6 Assets</div>
            <div className="text-[10px] text-slate-500 mt-0.5">BTC, ETH, SOL, AAPL, NVDA, MSFT</div>
          </div>
          ...
          <div className="p-3.5 rounded-2xl bg-slate-900/60 border border-slate-800/60">
            ...
            <div className="text-[10px] text-slate-500 mt-0.5">{scanResult ? 'Latest cycle complete' : 'Awaiting first scan'}</div>
          </div>

          <div className="p-3.5 rounded-2xl bg-slate-900/60 border border-slate-800/60">
            <div className="text-[10px] font-semibold text-emerald-400 uppercase tracking-wider">Successful</div>
            <div className="text-lg font-black text-emerald-300 mt-1">{scanResult ? scanResult.successfulCount : 0}</div>
            <div className="text-[10px] text-emerald-500/70 mt-0.5">Authoritative data feeds</div>
          </div>

          <div className="p-3.5 rounded-2xl bg-slate-900/60 border border-slate-800/60">
            <div className="text-[10px] font-semibold text-rose-400 uppercase tracking-wider">Failed Feeds</div>
            <div className="text-lg font-black text-rose-300 mt-1">{scanResult ? scanResult.failedCount : 0}</div>
            <div className="text-[10px] text-rose-500/70 mt-0.5">Isolated explicitly</div>
          </div>

          <div className="p-3.5 rounded-2xl bg-slate-900/60 border border-slate-800/60">
            <div className="text-[10px] font-semibold text-indigo-400 uppercase tracking-wider">Candidates Found</div>
            <div className="text-lg font-black text-indigo-300 mt-1">{scanResult ? scanResult.candidates.length : 0}</div>
            <div className="text-[10px] text-indigo-400/70 mt-0.5">Top-N score ranked</div>
          </div>

          <div className="p-3.5 rounded-2xl bg-slate-900/60 border border-slate-800/60">
            <div className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Last Scan</div>
            <div className="text-sm font-bold text-white mt-1.5 truncate">
              {scanResult ? new Date(scanResult.timestamp).toLocaleTimeString() : '—'}
            </div>
            <div className="text-[10px] text-slate-500 mt-0.5">Deterministic snapshot</div>
          </div>
        </div>
      </div>
      ...
      {errorMsg && (
        <div className="p-4 rounded-2xl bg-rose-950/40 border border-rose-500/40 text-xs text-rose-300 flex items-start gap-3 shadow-lg">
          ...
          <div>
            <strong className="text-rose-200 block mb-0.5">Scanner Error:</strong>
            <span>{errorMsg}</span>
          </div>
        </div>
      )}
      ...
      {!scanResult && !isScanning && (
        <div className="p-12 rounded-3xl bg-[#11141d] border border-slate-800 text-center space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {scanResult.candidates.map((cand) => {
              ...
              return (
                <div
                  key={cand.symbol}
                  className="p-6 rounded-3xl bg-[#11141d] border border-slate-800 hover:border-slate-700 transition space-y-5 relative shadow-xl"
                >
                  {/* Top Bar: Rank, Symbol, AssetClass, Queue Status */}
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-indigo-600 to-purple-600 text-white font-black text-sm flex items-center justify-center shadow-md">
                        #{cand.rank}
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                  ...
                  <div className="grid grid-cols-2 gap-3 p-3.5 rounded-2xl bg-slate-950/60 border border-slate-800/80">
                    <div>
                      <div className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Opportunity Score</div>
                      <div className="text-xl font-black text-indigo-400 mt-0.5">
                        {cand.score.toFixed(1)} <span className="text-xs text-slate-500 font-normal">/ 100</span>
                      </div>
                    ...
                    <div>
                      ...
                      <div className="text-xl font-black text-rose-400 mt-0.5">
                        {cand.signals.riskScore.toFixed(0)} <span className="text-xs text-slate-500 font-normal">/ 100</span>
                      </div>
                      <div className="w-full bg-slate-800 h-1.5 rounded-full mt-2 overflow-hidden">
                        <div
                  ...
                  <div className="grid grid-cols-3 gap-2.5 text-xs">
                    <div className="p-2.5 rounded-xl bg-slate-900/60 border border-slate-800/60">
                      ...
                      <div className={`font-bold text-xs mt-0.5 ${cand.signals.momentum >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                        {cand.signals.momentum >= 0 ? `+${cand.signals.momentum.toFixed(1)}%` : `${cand.signals.momentum.toFixed(1)}%`}
                      </div>
                    </div>

                    <div className="p-2.5 rounded-xl bg-slate-900/60 border border-slate-800/60">
                      <div className="text-[10px] text-slate-400">RVOL</div>
                    ...
                    <div className="p-2.5 rounded-xl bg-slate-900/60 border border-slate-800/60">
                      <div className="text-[10px] text-slate-400">Vol Accel</div>
                      <div className="font-bold text-white text-xs mt-0.5">
                  ...
                  <div className="p-3.5 rounded-2xl bg-indigo-950/20 border border-indigo-500/20 text-xs space-y-1.5">
                  ...
                  {inv && (
                    <div className="pt-3 border-t border-slate-800 flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="text-[11px] text-slate-400">Council Verdict:</span>
                        ...
                        {inv.decision?.riskGateApproved ? (
                          <span className="text-[10px] text-emerald-400 font-bold">Risk Gate: PASS</span>
                        ) : (
                          <span className="text-[10px] text-rose-400 font-bold">Risk Gate: BLOCKED</span>
                        )}
                        {inv.execution && (
        ...
        <div className="p-6 rounded-3xl bg-rose-950/20 border border-rose-500/30 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
            {scanResult.failedTargets.map((ft, i) => (
              <div key={i} className="p-3 rounded-2xl bg-slate-900/80 border border-rose-500/20 text-xs">
                <div className="font-bold text-white">${ft.symbol}</div>
                <div className="text-rose-400 text-[11px] mt-0.5">{ft.error}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* 5. Watchlist Section */}
      <div className="p-6 rounded-3xl bg-[#11141d] border border-slate-800 space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
        ...
        <div className="p-3 rounded-2xl bg-amber-500/5 border border-amber-500/20 text-[11px] text-amber-300/80 flex items-start gap-2">
            ...
            {watchlist.map((item) => (
              ...
              >
                ...
                <button
                  ...
                  className="text-xs text-slate-500 hover:text-rose-400 p-1.5 rounded-lg hover:bg-slate-800 transition"
